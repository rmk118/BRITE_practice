...
dat <- dat %>% mutate(new_col=c(1:1000), .before=R)
...
red_time_series<-ggplot(dat, aes(x=new_col, y=Red2))+geom_line()+theme_classic()

rho_E2 <- EmbedDimension(dataFrame = dat, columns = "Red2", target = "Red2",lib = "1 500", pred = "501 1000", showPlot = TRUE)
plot(rho_E2, ylim=(0.))
rho_E2[which.max(rho_E2$rho), "E"] #7 for red

rho_E3 <- EmbedDimension(dataFrame = dat, columns = "Logi2", target = "Logi2",lib = "1 500", pred = "501 1000", showPlot = TRUE)
rho_E3[which.max(rho_E3$rho), "E"] #7 for red
